...
if (!fixed_theta) {
    ...
    df <- data.frame(true_alpha_a_rew, true_alpha_a_pun, true_alpha_K, true_alpha_omega_f, true_alpha_omega_p,
                infer_alpha_a_rew, infer_alpha_a_pun, infer_alpha_K, infer_alpha_omega_f, infer_alpha_omega_p)
}

# save df
savefile_name = paste0("param_recovery_group_comparisons_", if(fixed_theta) "fixed_theta", ".csv")
write.csv(df, file.path(root_path, "ChatGPT-IGT", "src", "recovery", "results", savefile_name))
